...
export async function generateInitialSlides(projectId: string) {
  ...
  const [{ data: project }, { data: photos }, { data: refs }, { data: renders }, { data: views }] = await Promise.all([
    supabase.from("projects").select("name, type, subtype, location, clients(name), workspaces(name, address, phone)").eq("id", projectId).returns<Array<{ name: string; type: string; subtype: string | null; location: string | null; clients: { name: string } | { name: string }[] | null; workspaces: { name: string; address: string | null; phone: string | null } | null }>>().single(),
  ...
  const initial: Array<{ position: number; type: typeof slideTypes[number]; content_json: Record<string, unknown> }> = [
    ...
    { position: 3, type: "plan", content_json: { variant: "current_no_dims" } },
    { position: 4, type: "separator", content_json: { text: "PLANTA ATUAL COM MEDIDAS" } },
    { position: 5, type: "plan", content_json: { variant: "current_with_dims" } },
    ...
    { position: 11, type: "plan", content_json: { variant: "proposal" } },
